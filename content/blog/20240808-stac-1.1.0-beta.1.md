...
Additionally, [Item Asset Definition](#item-asset-defintions) (field `item_assets`) - formerly a popular STAC extension - is part of the core specification now.
...
We collaborated closely with the editors of OGC API - Records to align better with STAC, which resulted in a change to the [`license`](#license) field for example.
The [link object](#links) got extended to support additional HTTP mechanisms such as other methods than `GET` and headers.
...
This is a beta-release with the option to make changes before a final 1.1.0 release if the feedback of the STAC community asks for it in the next weeks. Currently, no further changes are planned at the time of this release though. You can find the [open issues for 1.1.0](https://github.com/radiantearth/stac-spec/milestone/33) in the GitHub issue tracker if any get submitted in the next weeks.

We'd appreciate if the STAC community takes some time to implement the new version and give feedback via the [GitHub issue tracker](https://github.com/radiantearth/stac-spec/issues) so that we can ensure the new version is mature and well-received by the community. We hope that the STAC ecosystem catches up with the changes in the specification in the next weeks and months, any help that you can provide is highly appreciated. If you want to fund the STAC work we do, please get in touch, too.

A shoutout to all the participants and sponsors of the last STAC sprint in Philadelphia, who laid down a solid basis for this release.
...
Usually, you can merge the each object on a by-index basis.
...
To better align with OGC API - Records, we changed the `license` field slightly.
...
The Link Object that is used for the `links` field, has been extended to support more HTTP mechanisms.
The additions were already specified in STAC API 1.0.0 but were forgotten to be added to STAC 1.0.0, so we catch up now.
...
- `headers`: The HTTP headers to send alonside the link request
- `body`: The request body to send for the link request (usually only applies for `POST`, `PUT` and `PATCH` methods)
...
The `item_assets` field that was formerly an [extension](https://github.com/stac-extensions/item-assets) has been upgraded to be part of the STAC specification directly.
It was probably the most commonly used extension and many extensions were defining schemas for it, so it is just simpler to have it in the core specification.
...
It doesn't really hurt to keep the schema URI though and although the extension has been deprecated it can still be used forever in STAC 1.0.0. It just won't get any updates in the future, because any changes will be directly integrated into STAC itself.
...
- Collection IDs should be unique across all collections in the corresponding root catalog and that Item IDs should be unique per collection
...
I already mentioned the changes in Item Assets, EO and Raster extensions above,
but we also released a significant change to the Projection extension.

The change is to allow more authorities for CRS codes.
Previously only EPSG was supported via the `proj:epsg` field, e.g. `"proj:egsp": 3857` for Web Mercator.
The new version replaces the field with `proj:code` so that the authority is included in the code, e.g. `"proj:code": "EPSG:3857"`.
This allows to provide codes from other authorities such as OGC or IAU.
